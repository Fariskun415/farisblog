...
$id_artikel = $_GET['id']; // ID artikel yang diinginkan dari URL
...
<head>
  ...
  <title>Faris Blog-Single Post</title>
  ...
  <style>
        ...
        .single-post {
            width: 100%; /* Lebar konten artikel, bisa disesuaikan */
            margin: auto; /* Pusatkan konten di tengah */
        }
    </style>
</head>
...
// Ambil id artikel dari URL atau parameter request lainnya
$id_artikel = $_GET['id'] ?? 1; // Menggunakan id=1 sebagai fallback

// Query untuk mengambil artikel berdasarkan id
$query = "SELECT a.*, k.nama_kategori FROM artikel a 
          JOIN kategori k ON a.id_kategori = k.id_kategori 
          WHERE a.id_artikel = ?";
...
if ($row = $result->fetch_assoc()) {
    ...
    <div class="single-post">
        <div class="post-meta">
            <span class="date"><?php echo htmlspecialchars($row['nama_kategori']); ?></span> 
            <span class="mx-1">&bullet;</span> 
            <span><?php echo date('M jS \'y', strtotime($row['tanggal'])); ?></span>
        </div>
        <h1 class="mb-5"><?php echo htmlspecialchars($row['judul']); ?></h1>
        
        <!-- Tampilkan isi artikel dengan format HTML dari CKEditor -->
                <?php if ($row['gambar']): ?>

            <figure class="my-4">

                <img src="data:image/jpeg;base64,<?php echo base64_encode($row['gambar']); ?>" alt="" class="img-fluid">
    ...
    <div class="aside-block">
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="pills-latest" role="tabpanel" aria-labelledby="pills-latest-tab">
                <?php
                $sql = "SELECT artikel.id_artikel, kategori.nama_kategori, artikel.judul, artikel.tanggal, artikel.penulis 
                        FROM artikel 
                        JOIN kategori ON artikel.id_kategori = kategori.id_kategori 
                        ORDER BY artikel.tanggal DESC 
                        LIMIT 6"; // Batas artikel yang ditampilkan
                $result = $conn->query($sql);

                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo '<div class="post-entry-1 border-bottom">';
                        echo '<div class="post-meta"><span class="date">' . htmlspecialchars($row["nama_kategori"]) . '</span> <span class="mx-1">&bullet;</span> <span>' . date("M jS 'y", strtotime($row["tanggal"])) . '</span></div>';
